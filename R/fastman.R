...
#' @param ps A string for the header of the column of base pair information in data.
#' @param p A string for the header for the column of P-value in data.
#' @param main A string for the Title of the plot.
#' @param suggest_line Set FALSE to remove the suggestive threshold line (p = 1e-5).
#' @param gws_line Set FALSE to remove the genome-wide significant line (p = 5e-8).
...
#' @export
fastman <- function(data, chr="CHR", ps="BP", p="P", main="fastman", suggest_line=T, gws_line=T, turbo=T, color=c("grey","black"), chr_build="GRCh37", yscale=NA){
    ...
    # When turno option is TRUE, set the min of yscale to 1 because all P <=0.1 are removed.
    y_min <- 0
    if(turbo){
        data <- data[which(data[,p]<=0.1),]
        y_min <- 1
    }
    # Restrict the number of SNPs with P > 0.1 to ~20000 by removing extras.
    if(!turbo){
    ...
    # Restrict the number of SNPs with P <=0.1 and P > 0.01 to ~20000 by removing extras.
    ...
    # Restrict the number of SNPs with P <=0.01 and P > 0.001 to ~20000 by removing extras.
    ...
    # Select whether the SNPs location info is based on GRCh38 or GRCh38
    if(chr_build=="GRCh37")
        chr_base <- c(0, 249250621, 492449994, 690472424, 881626700, 1062541960, 1233657027, 1392795690, 1539159712, 1680373143, 1815907890, 1950914406, 2084766301, 2199936179, 2307285719, 2409817111, 2500171864, 2581367074, 2659444322, 2718573305, 2781598825, 2829728720, 2881033286, 3036303846)
    ...
    if(chr_build=="GRCh38"){
        chr_base <- c(0, 248956422, 491149951, 689445510, 879660065, 1061198324, 1232004303, 1391350276, 1536488912, 1674883629, 1808681051, 1943767673, 2077042982, 2191407310, 2298451028, 2400442217, 2490780562, 2574038003, 2654411288, 2713028904, 2777473071, 2824183054, 2875001522, 3031042417)
    ...
    # Check whether any SNP is on sex chromosome. If so, plot them.
    chr_n <- c(1:22)
    xlabel <- c(1:22)
    x_at <- chr_label[1:22]
    ...
    axis(side = 2, at=c(y_min:y_max))
    mtext(expression(-log[10](P-value)), side = 2, line = 2)
    axis(side=1, line=-1, at=x_at, labels=xlabel, tick=F)
    ...
    if(suggest_line==T){
        abline(h=-log10(1*10^-5), col="blue", lty=2)
    }
}
